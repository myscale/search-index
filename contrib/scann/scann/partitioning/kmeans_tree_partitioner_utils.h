...
KMeansTreePartitionerFactoryPreSampledAndProjected(
  ...
  auto& cl = config.num_children_per_level();
  std::vector<int32_t> num_children_per_level(cl.begin(), cl.end());
  KMeansTreeConfig tree_config{config.num_children(), std::move(num_children_per_level)};
  SCANN_RETURN_IF_ERROR(result->CreatePartitioning(
      *dataset, *training_dist, tree_config, &opts));
