...
Status KMeansTreePartitioner<T>::CreatePartitioning(
    ...
    const KMeansTreeConfig& config, KMeansTreeTrainingOptions* opts) {
  if (kmeans_tree_) {
  ...
  SCANN_RETURN_IF_ERROR(
      tree->Train(training_dataset, training_dist, config, opts));
...
Status KMeansTreePartitioner<T>::TokensForDatapointWithSpilling(
    const DatapointPtr<T>& dptr, MaxSpillingConfig max_spilling_config,
    vector<KMeansTreeSearchResult>* result) const {
  DCHECK(result);
  VLOG(1) << "TokensForDatapointWithSpilling max_centers_override="
          << max_spilling_config.max_spilling_centers
          << " max_l0_children_to_search="
          << max_spilling_config.max_l0_children_to_search
          << " query_spilling_max_centers="
          << query_spilling_max_centers_;

  if (this->tokenization_mode() == UntypedPartitioner::QUERY) {
    const auto config = max_spilling_config.valid()
                        ? max_spilling_config : MaxSpillingConfig();

    if (query_tokenization_type_ == ASYMMETRIC_HASHING) {
      int pre_reordering_num_neighbors =
          TokenizationSearcher()->reordering_enabled()
              ? config.max_spilling_centers * kAhMultiplierSpilling
              : config.max_spilling_centers;
      return TokensForDatapointWithSpillingUseSearcher(
          dptr, result, config.max_spilling_centers, pre_reordering_num_neighbors);
    }

    return kmeans_tree_->Tokenize(
        dptr, *query_tokenization_dist_,
        KMeansTree::TokenizationOptions::UserSpecifiedSpilling(
            query_spilling_type_, query_spilling_threshold_, config,
      ...
      if (database_tokenization_type_ == ASYMMETRIC_HASHING) {
        ...
        return kmeans_tree_->Tokenize(
            ...
            KMeansTree::TokenizationOptions::UserSpecifiedSpilling(
                ...
                MaxSpillingConfig(database_spilling_fixed_number_of_centers_, -1),
...
Status KMeansTreePartitioner<T>::TokensForDatapointWithSpillingAndOverride(
  ...
  SCANN_RETURN_IF_ERROR(
      TokensForDatapointWithSpilling(dptr, max_spilling_config, &result_raw));
  result->clear();
...
KMeansTreePartitioner<T>::TokensForDatapointWithSpillingBatchedAndOverride(
    const TypedDataset<T>& queries, ConstSpan<MaxSpillingConfig> max_spilling_configs,
  ...
  SCANN_RETURN_IF_ERROR(TokensForDatapointWithSpillingBatched(
      queries, max_spilling_configs, MakeMutableSpan(raw_results)));
...
Status KMeansTreePartitioner<T>::TokensForDatapointWithSpillingBatched(
  ...
  if (!max_spilling_configs.empty() &&
      queries.size() != max_spilling_configs.size())
    return InvalidArgumentError(
  ...
  if (this->tokenization_mode() != UntypedPartitioner::QUERY ||
    ...
    for (DatapointIndex i = 0; i < queries.size(); ++i) {
      const auto max_spilling_config =
          max_spilling_configs.empty() ? MaxSpillingConfig() : max_spilling_configs[i];
      SCANN_RETURN_IF_ERROR(
          TokensForDatapointWithSpilling(queries[i], max_spilling_config, &results[i]));
    }
    return OkStatus();
  }
  ...
  if (query_spilling_type_ == QuerySpillingConfig::FIXED_NUMBER_OF_CENTERS) {
    ...
    for (DatapointIndex query_idx : IndicesOf(*float_queries)) {
      const auto max_centers = max_spilling_configs.empty()
                                   ? query_spilling_max_centers_
                                   : max_spilling_configs[query_idx].max_spilling_centers;
