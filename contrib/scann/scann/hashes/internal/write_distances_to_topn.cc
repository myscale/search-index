...
void WriteDistancesToTopNImpl(const RestrictAllowlist* whitelist_or_null,
                              ConstSpan<DatapointIndex> local_to_global_index_map,
  ...
  if (restricts_enabled) {
    whitelist_view = RestrictAllowlistConstView(*whitelist_or_null, local_to_global_index_map);
  }
  ...
  auto block_contains_whitelisted_8 =
    ...
    if (whitelist_or_null->isDirectList()) {
      ...
      // if not direct mapping, need to iterate over each data point and might be slower
      for (DatapointIndex j=dp_index; j<dp_index+8; ++j)
        if (whitelist_or_null->IsWhitelisted(j)) return true;
      return false;
    }
  };
...
void WriteDistancesToTopN(const RestrictAllowlist* whitelist_or_null,
  ...
  return (!whitelist_or_null)
             ? WriteDistancesToTopNImpl<false>(whitelist_or_null, local_to_global_index_map,
                                               max_distance, distances, top_n)
             : WriteDistancesToTopNImpl<true>(whitelist_or_null, local_to_global_index_map,
                                              max_distance, distances, top_n);
}
