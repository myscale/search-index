...
class Searcher final : public SingleMachineSearcherBase<T> {
 ...
 private:
  ...
  // map from local index to global filter positions (cached for speed)
  mutable vector<uint32_t> index_to_filter_pos;
  mutable std::atomic<bool> index_to_filter_pos_empty{true};
  mutable std::mutex index_to_filter_pos_mutex;
