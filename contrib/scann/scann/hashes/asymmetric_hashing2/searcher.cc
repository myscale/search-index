...
Searcher<T>::Searcher(shared_ptr<TypedDataset<T>> dataset,
  ...
  if (lut16_) {
    packed_dataset_ =
        ::research_scann::asymmetric_hashing2::CreatePackedDataset(
            *this->hashed_dataset(),
            const_cast<DenseDataset<uint8_t>*>(this->hashed_dataset_packed()),
            opts_.num_blocks());
  ...
  if (lut16_) {
    // for lut16, we use packed dataset for further processing
    // releasing hashed_dataset to save memory
    this->ReleaseHashedDataset(); 
  }
}
...
Status Searcher<T>::FindNeighborsTopNDispatcher(
  ...
  if (params.pre_reordering_crowding_enabled()) {
    ...
    auto whitelist = params.get_restrict_whitelist();
    auto index_map = params.get_local_to_global_index_map();
    if (params.restricts_enabled() && whitelist && !index_map.empty()) {
      if (index_to_filter_pos_empty) {
        std::lock_guard<std::mutex> lock(index_to_filter_pos_mutex);
        // other threads might have mutated the atomic bool at the same time
        if (index_to_filter_pos_empty) {
          CHECK(this->index_to_filter_pos.empty());
          this->index_to_filter_pos.resize(index_map.size());
          auto id_list = whitelist->getIDList();
          for (int i = 0; i < index_map.size(); ++i) {
            auto pos = id_list[index_map[i]];
            CHECK_LT(pos, 1UL << 32);
            this->index_to_filter_pos[i] = pos;
          }
          index_to_filter_pos_empty = false;
        }
      }
      // store the computed map in the params
      const_cast<SearchParameters&>(params)
        .set_local_idx_to_global_pos_map(this->index_to_filter_pos);
    }
  ...
  if (opts_.asymmetric_queryer_) {
    ...
    *opts.ah_codebook =
    ...
    if (opts_.asymmetric_lookup_type_ == AsymmetricHasherConfig::INT8_LUT16) {
      ...
      // remove the existing int8 dataset if there is one
      opts.hashed_dataset.reset();
      opts.hashed_dataset_creator = [this]() {
        // creating the hashed dataset on the fly to save memory
        VLOG(1) << "Creating hashed_dataset";
        return make_shared<DenseDataset<uint8_t>>(UnpackDataset(packed_dataset_));
      };
    }
  }
  return opts;
