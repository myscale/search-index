...
class UntypedSingleMachineSearcherBase {
 public:
  ...
  shared_ptr<const DenseDataset<uint8_t>> hashed_dataset_by_token() const {
    return hashed_dataset_by_token_;
  }

  void set_hashed_dataset_by_token(shared_ptr<const DenseDataset<uint8_t>> dataset) {
    hashed_dataset_by_token_ = dataset;
  }

  const DenseDataset<uint8_t>* hashed_dataset_packed() const {
    return hashed_dataset_packed_.get();
  }
 ...
 private:
  UntypedSingleMachineSearcherBase(
      ...
      shared_ptr<const DenseDataset<uint8_t>> hashed_dataset_by_token_,
      shared_ptr<const DenseDataset<uint8_t>> hashed_dataset_packed_,
  ...
  // order hashed_data by token to save memory
  shared_ptr<const DenseDataset<uint8_t>> hashed_dataset_by_token_ = nullptr;
  // packed hashed dataset
  shared_ptr<const DenseDataset<uint8_t>> hashed_dataset_packed_ = nullptr;
...
class SingleMachineSearcherBase : public UntypedSingleMachineSearcherBase {
 public:
  ...
  SingleMachineSearcherBase(int32_t default_pre_reordering_num_neighbors,
                            float default_pre_reordering_epsilon)
      : UntypedSingleMachineSearcherBase(nullptr, nullptr,
