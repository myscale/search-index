...
class SearchParameters {
 public:
  ...
  bool restricts_enabled() const { return restrict_whitelist_ != nullptr; }

  void set_restrict_whitelist(shared_ptr<RestrictAllowlist> whitelist) {
      restrict_whitelist_ = whitelist; }

  const RestrictAllowlist* restrict_whitelist() const { return restrict_whitelist_.get(); }

  bool IsWhitelisted(DatapointIndex dp_index) const { return false; }

  RestrictAllowlist* mutable_restrict_whitelist() { return restrict_whitelist_.get(); }

  std::shared_ptr<RestrictAllowlist> get_restrict_whitelist() const { return restrict_whitelist_; }

  void set_local_to_global_index_map(ConstSpan<DatapointIndex> index_map) {
    local_to_global_index_map_ = index_map;
  }

  ConstSpan<DatapointIndex> get_local_to_global_index_map() const {
    return local_to_global_index_map_;
  }

  void set_local_idx_to_global_pos_map(ConstSpan<uint32_t> pos_map) {
    local_idx_to_global_pos_map_ = pos_map;
  }

  ConstSpan<uint32_t> get_local_idx_to_global_pos_map() const {
    return local_idx_to_global_pos_map_;
  }
 ...
 private:
  ...
  shared_ptr<RestrictAllowlist> restrict_whitelist_{nullptr};
  ConstSpan<DatapointIndex> local_to_global_index_map_;
  ConstSpan<uint32_t> local_idx_to_global_pos_map_;
};
