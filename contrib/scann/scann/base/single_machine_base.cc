...
UntypedSingleMachineSearcherBase::UntypedSingleMachineSearcherBase(
    ...
    : hashed_dataset_(hashed_dataset),
      hashed_dataset_by_token_(hashed_dataset_by_token),
      hashed_dataset_packed_(hashed_dataset_packed),
...
SingleMachineSearcherBase<T>::SingleMachineSearcherBase(
    ...
    : SingleMachineSearcherBase(dataset, nullptr, nullptr,
                                /*hashed_dataset_packed*/ nullptr,
                                default_pre_reordering_num_neighbors,
                                default_pre_reordering_epsilon) {}
...
SingleMachineSearcherBase<T>::ExtractSingleMachineFactoryOptions() {
  ...
  opts.hashed_dataset_by_token =
      std::const_pointer_cast<DenseDataset<uint8_t>>(hashed_dataset_by_token_);
...
Status SingleMachineSearcherBase<T>::FindNeighbors(
  ...
  if (VLOG_IS_ON(1)) {
    LOG_EVERY_N(INFO, 100)
      << "SingleMachineSearcherBase<T>::FindNeighbors"
      << " find_neighbors_noexact=" << size_t(dt0.count()) / 1000.0f
      << " reorder_results=" << size_t(dt1.count()) / 1000.0f;
  }
...
Status SingleMachineSearcherBase<T>::FindNeighborsBatched(
  ...
  auto t1 = std::chrono::high_resolution_clock::now();
  auto num_reorder = results[0].size();
  if (reordering_helper_) {
  ...
  auto time_diff_ms = [](auto ta, auto tb) {
    return std::chrono::duration_cast<std::chrono::milliseconds>(tb - ta).count();
  };
  VLOG(1) << "SingleMachineSearcherBase<T>::FindNeighborsBatched"
          << " find_neighbors_noexact=" << time_diff_ms(t0, t1)
          << " reorder_results=" << time_diff_ms(t1, t2) << " num_reorder=" << num_reorder
          << " sort_and_drop_results=" << time_diff_ms(t2, t3);
...
Status SingleMachineSearcherBase<T>::FindNeighborsBatchedImpl(
    const TypedDataset<T>& queries, ConstSpan<SearchParameters> params,
    MutableSpan<NNResultsVector> results) const {
  VLOG(1) << "SingleMachineSearcherBase<T>::FindNeighborsBatchedImpl queries=" << queries.size();
