...
class Avx2<T, kNumRegistersInferred> {
 public:
  ...
  template<int count>
  static SCANN_AVX2_INLINE IntelType ShiftRight(IntelType x) {
  ...
  template<int count>
  SCANN_AVX2_INLINE Avx2 operator>>(std::integral_constant<int, count>) const {
    const Avx2& me = *this;
    Avx2 ret;
    for (size_t j : Seq(kNumRegisters)) {
  ...
  template<int count>
  static SCANN_AVX2_INLINE IntelType ShiftLeft(IntelType x) {
    static_assert(!IsSameAny<T, int8_t, uint8_t>(),
                  "There's no 8-bit '<<' instruction");
    static_assert(!IsSameAny<T, float, double>(),
                  "Bit shifting isn't defined for floating-point types.");
  ...
  template<int count>
  SCANN_AVX2_INLINE Avx2& operator<<=(std::integral_constant<int, count>) {
    Avx2& me = *this;
    for (size_t j : Seq(kNumRegisters)) {
