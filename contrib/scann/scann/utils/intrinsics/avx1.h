...
class Avx1<T, kNumRegistersInferred> {
 public:
  ...
  template<int count>
  static SCANN_AVX1_INLINE IntelType ShiftRight(IntelType x) {
  ...
  template<int count>
  SCANN_AVX1_INLINE Avx1 operator>>(std::integral_constant<int, count>) const {
    const Avx1& me = *this;
    Avx1 ret;
    for (size_t j : Seq(kNumRegisters)) {
      ret[j] = ShiftRight<count>(*me[j]);
    }
    return ret;
  }

  template<int count>
  static SCANN_AVX1_INLINE IntelType ShiftLeft(IntelType x) {
    static_assert(!IsSameAny<T, int8_t, uint8_t>(),
                  "There's no 8-bit '<<' instruction");
    static_assert(!IsSameAny<T, float, double>(),
                  "Bit shifting isn't defined for floating-point types.");
  ...
  template<int count>
  SCANN_AVX1_INLINE Avx1& operator<<=(std::integral_constant<int, count>) {
    Avx1& me = *this;
    for (size_t j : Seq(kNumRegisters)) {
      me[j] = ShiftLeft<count>(*me[j]);
    }
    return *this;
  }
...
#ifdef __x86_64__
    // _mm256_cvtepi64_pd is only available on x86_64
