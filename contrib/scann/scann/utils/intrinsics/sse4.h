...
class Sse4<T, kNumRegistersInferred> {
 public:
  ...
  template<int count>
  static SCANN_SSE4_INLINE IntelType ShiftRight(IntelType x) {
  ...
  template<int count>
  SCANN_SSE4_INLINE Sse4 operator>>(std::integral_constant<int, count>) const {
    const Sse4& me = *this;
    Sse4 ret;
    for (size_t j : Seq(kNumRegisters)) {
  ...
  template<int count>
  static SCANN_SSE4_INLINE IntelType ShiftLeft(IntelType x) {
    static_assert(!IsSameAny<T, int8_t, uint8_t>(),
                  "There's no 8-bit '<<' instruction");
    static_assert(!IsSameAny<T, float, double>(),
                  "Bit shifting isn't defined for floating-point types.");
  ...
  template<int count>
  SCANN_SSE4_INLINE Sse4& operator<<=(std::integral_constant<int, count>) {
    Sse4& me = *this;
    for (size_t j : Seq(kNumRegisters)) {
