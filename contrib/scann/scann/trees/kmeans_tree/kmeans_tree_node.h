...
struct KMeansTreeConfig {
  int32_t k_per_level = -1;
  // when not empty, override k_per_level
...
class KMeansTreeNode {
 ...
 private:
  ...
  void BuildFromProto(const SerializedKMeansTree::Node& proto, int32_t node_level);
...
Status KMeansTreeNode::FindChildrenWithSpilling(
    ...
    std::vector<pair<DatapointIndex, float>>* child_centers) const {
  VLOG(1) << "KMeansTreeNode::FindChildrenWithSpilling node_level=" << node_level_;
