...
ScannNumpy::SearchBatched(const np_row_major_arr<float>& queries, int final_nn,
  ...
  VLOG(1) << "ScannNumpy::SearchBatched single_query_mode=" << single_query_mode;
  if (queries.ndim() != 2)
  ...
  Status status;
  if (parallel) {
    CHECK(!single_query_mode) << "single_query_mode is only supported in non-parallel batch search";
    ...
    status = scann_.SearchBatched(query_dataset, MakeMutableSpan(res), final_nn,
                                  pre_reorder_nn, leaves, single_query_mode);
  RuntimeErrorIfNotOk("Error during search: ", status);
