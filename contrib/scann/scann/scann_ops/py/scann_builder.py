...
class ScannBuilder(object):
  ...
  def tree(
      ...
      compute_residual_stdev=False,
      ):
    stdev_str = f"compute_residual_stdev: {compute_residual_stdev}"
    return f"""
      partitioning {{
        ...
        {stdev_str if compute_residual_stdev else ""}
      }}
    """
  ...
  def score_ah(
      ...
      use_normalized_residual_quantization=False,
      # the following are set automatically
    ...
    nrq_str = \
      f"use_normalized_residual_quantization:{use_normalized_residual_quantization}"
    return f"""
      hash {{
        asymmetric_hash {{
          ...
          {nrq_str if not global_topn and use_normalized_residual_quantization else ""}
  ...
  def create_config(self):
    ...
    if ah is not None and tree_params is not None \
      and 'use_normalized_residual_quantization' in ah:
      tree_params['compute_residual_stdev'] = ah['use_normalized_residual_quantization']
    if ah is not None and bf is None:
      if ah.get("residual_quantization") is None:
